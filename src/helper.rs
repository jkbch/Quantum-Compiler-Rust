...
pub enum Value {
    Scalar(Scalar),
    Array(Array), // element type + optional size
}

#[derive(Debug, Clone, Copy, PartialEq)]
pub enum Ty {
    Scalar(Type),
    Array(Type, Option<i64>), // element type + optional size
}

pub type ValueEnv = Env<Option<Value>>;
pub type TypeEnv = Env<Ty>;
pub type ProcMap = HashMap<String, Procedure>;

pub fn type_rank(t: Type) -> i32 {
    match t {
        Type::Cbit => 0,
        Type::Int => 1,
        Type::Float => 2,
...
pub fn max_type(t1: Type, t2: Type) -> Type {
...
impl<T: Clone> Env<T> {
    ...
    pub fn push_scope(&mut self, entries: HashMap<String, T>) {
    ...
    pub fn update(&mut self, name: &str, val: T) {
...
pub fn decl_name(d: &Declaration) -> &String {
    match d {
...
pub fn param_decl_name(d: &ParameterDeclaration) -> &String {
    match d {
...
pub fn eval_fun_2(name: &str, arg1: Scalar, arg2: Scalar) -> Scalar {
    match (name, &arg1, &arg2) {
        ...
        (_, Scalar::Int(x), y) => eval_fun_2(name, Scalar::Float(*x as f64), *y),
        (_, x, Scalar::Int(y)) => eval_fun_2(name, *x, Scalar::Float(*y as f64)),
...
pub fn type_binop(op: &str, lhs: Type, rhs: Type) -> Type {
    match (&lhs, &rhs) {
        ...
        (Type::Int, Type::Float) => type_binop(op, Type::Float, Type::Float),
        (Type::Float, Type::Int) => type_binop(op, Type::Float, Type::Float),
        _ => panic!("Unsupported binary op {} for {:?} and {:?}", op, lhs, rhs),
    }
}
